...
pub fn get_cursor_position() -> MouseResult<(i32, i32)> {
    ...
    if result == 0 {
        return Err(MouseError::WinApiError(
...
pub fn validate_coordinates(x: i32, y: i32) -> MouseResult<()> {
    ...
    if x < 0 || x >= screen_width || y < 0 || y >= screen_height {
        return Err(MouseError::InvalidCoordinates(format!(
            "Coordinates ({}, {}) are outside screen bounds ({}x{})",
...
pub fn generate_human_like_path(
    ...
    let num_points =
        (settings.curve_smoothness.max(2) as f64 * (distance / 100.0).sqrt()).ceil() as usize;
